# RDS拡張モニタリング機能をcloudwatch logsに送信するIAM

resource "aws_iam_role" "rds_monitoring_role" {
    name = "${local.name_prefix}-${local.service_name}-rds-monitoring-role"
    ...
    tags = {
      "Name" = "${local.name_prefix}-${local.service_name}-rds-monitoring"
    }
}
